  ...
  return (
    <article className="group overflow-hidden rounded-lg border border-white/10 bg-white/[.035] shadow-2xl shadow-black/20 transition hover:-translate-y-1 hover:border-violet-400/40">
      <Link to={`/projects/${project.slug}`} className="block h-48 overflow-hidden" aria-label={`View ${project.title}`}>
        <div className="h-full w-full transition duration-500 group-hover:scale-105" style={{ background: project.thumbnail }} />
      </Link>
      <div className="p-5">
        <div className="mb-3 flex items-center justify-between gap-3">
          <span className="rounded-md border border-violet-400/30 bg-violet-500/10 px-2.5 py-1 text-xs font-semibold text-violet-200">{project.category.name}</span>

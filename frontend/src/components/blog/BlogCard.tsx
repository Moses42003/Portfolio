  ...
  return (
    <article className="overflow-hidden rounded-lg border border-white/10 bg-white/[.035] transition hover:-translate-y-1 hover:border-violet-400/40">
      <Link to={`/blog/${post.slug}`} className="block h-44" aria-label={`Read ${post.title}`}>
        <div className="h-full" style={{ background: post.cover_image ?? post.thumbnail_url ?? "linear-gradient(135deg, rgba(168,85,247,0.25), rgba(34,211,238,0.15))" }} />
      </Link>
      <div className="p-5">
        <div className="mb-3 flex flex-wrap gap-2">
          <span className="rounded-md bg-violet-500/15 px-2 py-1 text-xs font-semibold text-violet-200">{getCategoryName(post.category)}</span>
          <span className="rounded-md border border-white/10 px-2 py-1 text-xs text-slate-400">{post.reading_time ?? 5} min read</span>
        </div>

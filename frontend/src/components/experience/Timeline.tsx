        ...
        <article key={item.id} className="relative pl-10">
          ...
          <div className="rounded-lg border border-white/10 bg-white/[.035] p-6">
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              {item.technologies.map((technology) => <span key={technology} className="rounded-md border border-white/10 bg-slate-950/60 px-2 py-1 text-xs text-slate-300">{technology}</span>)}
            </div>
          </div>
        </article>
      ))}

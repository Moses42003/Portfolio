...
import { formatDate, getCategoryName, getProjectImage } from "../../lib/utils";
      ...
      {query.isLoading ? <LoadingState /> : (
        <AdminTable headers={["Thumbnail", "Title", "Category", "Status", "Featured", "Updated", "Actions"]}>
          {query.data?.map((project) => (
            <tr key={project.id}>
              <td className="px-4 py-4"><div className="size-12 rounded-md" style={{ background: getProjectImage(project) }} /></td>
              <td className="px-4 py-4 font-semibold text-white">{project.title}</td>
              <td className="px-4 py-4">{getCategoryName(project.category)}</td>
              <td className="px-4 py-4 capitalize">{project.status}</td>
              <td className="px-4 py-4">{project.featured ? "Yes" : "No"}</td>
              <td className="px-4 py-4">{project.updated_at ? formatDate(project.updated_at) : "—"}</td>

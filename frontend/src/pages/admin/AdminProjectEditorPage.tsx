...
export function AdminProjectEditorPage() {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<ProjectForm>({
    ...
    values: project.data ? { title: project.data.title, slug: project.data.slug, description: project.data.description, content: project.data.content ?? "", thumbnail: project.data.thumbnail, category_id: project.data.category.id, technology_ids: project.data.technologies.map((technology) => technology.id).join(","), github_url: project.data.github_url, live_url: project.data.live_url, featured: project.data.featured, status: project.data.status, start_date: project.data.start_date, end_date: project.data.end_date } : { title: "", slug: "", description: "", content: "", thumbnail: "", category_id: "cat-web", technology_ids: "tech-react,tech-ts", github_url: "", live_url: "", featured: false, status: "draft", start_date: "", end_date: "" },
  });
  ...
  return (
    <section className="pb-20 lg:pb-0">
      ...
      <form className="grid gap-5 rounded-lg border border-white/10 bg-white/[.035] p-5 lg:grid-cols-2" onSubmit={handleSubmit(async (values) => { const payload = { ...values, technology_ids: values.technology_ids.split(",").map((item) => item.trim()) }; if (isNew) await create.mutateAsync(payload); else await update.mutateAsync({ id: id ?? "", payload }); notify("Project saved.", "success"); navigate("/admin/projects"); })}>

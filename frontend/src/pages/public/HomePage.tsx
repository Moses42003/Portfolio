...
export function HomePage() {
  ...
  const stats = asArray(profile?.stats) as Array<Record<string, any>>;
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto grid min-h-[calc(100vh-5rem)] max-w-7xl items-center gap-12 px-4 py-16 sm:px-6 lg:grid-cols-[1fr_.9fr] lg:px-8">
          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            ...
            <p className="text-2xl font-bold text-white">{profile?.greeting ?? profile?.title ?? "Hi, I'm Moses"} <span aria-hidden="true">👋</span></p>
            <h1 className="mt-3 max-w-4xl text-5xl font-black leading-tight tracking-tight text-white sm:text-6xl lg:text-7xl">
              <span className="bg-gradient-to-r from-white via-violet-200 to-cyan-200 bg-clip-text text-transparent">{profile?.headline ?? profile?.summary ?? "I build digital experiences that make an impact."}</span>
            </h1>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <a href={profile?.cv_url ?? profile?.website ?? "#"} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-lg border border-white/15 bg-white/[.04] px-4 py-2 text-sm font-semibold text-slate-100 transition hover:border-violet-400/50">
                <ArrowDownToLine className="size-4" aria-hidden="true" /> Download CV
              </a>
            </div>
            <div className="mt-8 flex gap-3" aria-label="Social links">
              {[
                { href: profileSocials.github ?? profile?.github, label: "GitHub", icon: GitBranch },
                { href: profileSocials.linkedin ?? profile?.linkedin, label: "LinkedIn", icon: BriefcaseBusiness },
                { href: profileSocials.twitter, label: "Twitter/X", icon: Globe },
                { href: profileSocials.email ?? profile?.email, label: "Email", icon: Mail },
              ].map((item) => {
      ...
      <section className="mx-auto max-w-7xl px-4 pb-10 sm:px-6 lg:px-8">
        {profileQuery.isLoading ? <LoadingState label="Loading profile stats" /> : profileQuery.isError ? <ErrorState message="Unable to load profile statistics." /> : (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {stats.map((stat) => (
              <div key={stat.id ?? stat.label} className="rounded-lg border border-white/10 bg-white/[.035] p-6 text-center">
